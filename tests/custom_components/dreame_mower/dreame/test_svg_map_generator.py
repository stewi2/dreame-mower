...
class TestLiveModeGolden:
    ...
    def test_live_mode_mid_mowing(self, golden_map_data, live_mode_data, mock_coordinator):
        ...
        On the very first run the golden file does not exist yet; the test
        writes it and passes so the file can be committed as the reference.
        """
        ...
        # Save actual output for visual inspection
        actual_svg_file = TEST_DATA_DIR / "test_svg_live_mode_actual.svg"
        with open(actual_svg_file, "wb") as f:
            f.write(result)

        # Bootstrap: create the golden file on the first run
        if not LIVE_GOLDEN_SVG_FILE.exists():
            with open(LIVE_GOLDEN_SVG_FILE, "wb") as f:
                f.write(result)
        ...
        golden_normalised = _TIMESTAMP_RE.sub("Updated: TIMESTAMP", golden_svg)
